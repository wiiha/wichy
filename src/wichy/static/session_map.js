...
function showError(message) {
    ...
    banner.style.cssText = 'position:fixed;top:0;left:0;right:0;background:#fee;border:1px solid #f00;padding:10px;color:#900;z-index:1000;';
...
async function loadMap() {
    try {
        ...
        if (mapRes.ok) {
            ...
            const newDataStr = JSON.stringify({nodes: mapData.nodes, edges: mapData.edges});
...
function showAddNoteModal() {
    ...
    nodes.forEach(node => {
        ...
        const text = node.data.content.length > 50 
...
async function deleteSelectedNode() {
    ...
    try {
        const res = await fetch(`/tools/session-map/api/node/${selectedNode.id}`, { method: 'DELETE' });
        if (!res.ok) {
