...
class ToolManager:
    ...
    def __init__(self, all_tools: Optional[List[type[BaseTool]]] = None):
        ...
        """
        self.all_tools = all_tools if all_tools is not None else ALL_TOOLS_NOT_INSTANTIATED
    ...
    def filter_tools(
        ...
        if allowed:
            allowed_names = {name.strip().lower() for name in allowed.split(",") if name.strip()}
        ...
        # Apply exclusions
        if excluded:
            excluded_names = {name.strip().lower() for name in excluded.split(",") if name.strip()}
            filtered = [tool for tool in filtered if tool.name.lower() not in excluded_names]
