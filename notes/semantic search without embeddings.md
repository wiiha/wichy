...
In semantic search, content and query map to a **_shared representation_**. This space has a **_similarity_** function that scores similar items higher than less similar.
...
Now we have a shared _representation_: a three tuple of (`shape` , `color`, `item type`).
...
We’d prefer to to compute embeddings from **_features_** of the item. Then strawberry 🍓 could get an embedding right away. IE instead of `strawberry_embedding = embeddings['strawberry']` we can do `strawberry_embedding = f(heart_shaped, red, fruit)`. We build an **_encoder._** Given the item’s attributes we compute embeddings that produce the expected similarity. Embedding models downloaded from hugging face specialize in encoding specific type/domain of content for a specific task (search, etc).
...
One pain point comes up right away with embeddings: lack of _matching_.
...
One system tries to solve all three problems: _Managed vocabularies_ or _taxonomies_.
...
We have a _representation_ (the category tree), do we have a similarity function?
...
Score (due to doc freq)

Example of matches

“Baby & Kids”

Matches highest level category
...
Matches next most specific category Getting to “play” stuff for kids, better than just kid stuff. 👍

Low

**Baby & Kids / Toddler & Kids Playroom** / Playroom Furniture / Baby Gyms & Playmats / Folding Baby Gyms & Playmats
...
Matches a very, narrow specific category. Horses!🐴 Even if not “Novelty Rocking Horses” its a defensible “match” to the user.
...
You can tighten / relax in categories that matter to users, **_semantically,_** up a layer to still stay within the realm of “kid play”.
